# from utils.matrix_utils import add_to_row_col, matrix_without_row_col
...
def rec_partition_allsubgraphs(matrix, partition, tabs, count_trees=False):
    # print(tabs, 'partitions', matrix, partition)
    # Receives the matrix without the ignored nodes
    is_ended = has_ended(matrix, partition)
    # print(tabs, 'is_aneded', is_ended)
    if all((len(a) == 1 for a in partition)) and is_ended:
        # print(tabs, 'Caso A')
        return 1
    if is_ended:
        # print(tabs, 'retorna 0 por ended')
        return 0
    for a in partition:
        n = a[0]
        # print(tabs, 'n=', n)
            ...
            if matrix[n][i] != 0 and n != i and i not in excluded_nodes:
                ...
                for p in new_partition:
                    for j, u in enumerate(p):
                        if i < u:
                            p[j] = u - 1
                # print(tabs, 'borramos', n, i)
                f1 = rec_partition_allsubgraphs(
                    matrix, partition, tabs + '\t', count_trees)
                # print(tabs, 'colapsamos', n, i)
                f2 = rec_partition_allsubgraphs(
                    mt, new_partition, tabs + '\t', count_trees)
                if not count_trees:
                    return f1 + (2**v - 1)*f2
                return f1 + v * f2
    # print(tabs, 'retorna 0')
    return 0


def contraction_subgraph_count(matrix, distinguished, partition, count_trees=False):
    # print('coutntrees', count_trees)
    covered = [
        n for s in partition for n in s]
    excluded = list(filter(lambda n: n not in covered, distinguished))
    for n in excluded:
    ...
    while len(excluded) > 0:
        ...
        for k, _e in enumerate(excluded):
            if e < _e:
                excluded[k] = _e - 1
    return int(rec_partition_allsubgraphs(matrix, partition, '', count_trees))

def get_supremum(p1, p2, count_trees=False):
    ...
    for b in s2:
        for i, a in enumerate(s1):
            if len(a.intersection(b)):
                if count_trees and len(a.intersection(b)) > 1:
                    return None
                new_s[i] = a.union(b)
    if len(new_s) == 1:
...
def partition_repr(sets):
    sorted_sets = [[str(i) for i in sorted(l, key=lambda t: int(t))]
                   for l in sets]
    string_sets = sorted(['.'.join(l) for l in sorted_sets])
    return '-'.join(string_sets)


def format_d(d_set):
    return ".".join(map(str, sorted([int(i) for i in d_set])))
...
def compute_separated_bags(rtd, edge_values, values, centralities):
    pass


if __name__ == '__main__':
    a = []
    s = partition_repr([['10', '7']])
    print(s)
